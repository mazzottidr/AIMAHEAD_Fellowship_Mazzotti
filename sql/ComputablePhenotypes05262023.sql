...
with temp_aggregated as ( 	
	...
	select
		PATIENT_NUM,
		min(START_DATE) as earliest_Stroke_date,
		max(START_DATE) as latest_Stroke_date,
		count(distinct START_DATE) as N_Stroke_codes_diffdates
	from FellowsSample.S19.OBSERVATION_FACT
...
select
  ...
  --count(distinct t.PATIENT_NUM)
  	clean_ObsFact.PATIENT_NUM as PATIENT_NUM,
	clean_ObsFact.ENCOUNTER_NUM as earliest_Stroke_encounter_num,
	t.earliest_Stroke_date as earliest_Stroke_date,
	t.latest_Stroke_date as latest_Stroke_date,
	t.N_Stroke_codes_diffdates,
	clean_ObsFact.CONCEPT_CD as earliest_Stroke_code,
	clean_ObsFact.TVAL_CHAR as earliest_Stroke_TVAL_CHAR,
	1 as Stroke_ComputablePhenotype
into S19.dbo.Stroke_CompPheno
from clean_ObsFact
INNER JOIN temp_aggregated t on t.PATIENT_NUM = clean_ObsFact.PATIENT_NUM AND t.earliest_Stroke_date = clean_ObsFact.START_DATE
WHERE rn = 1
ORDER BY PATIENT_NUM, earliest_Stroke_date;

--drop table S19.dbo.Stroke_TEST;
--drop table S19.dbo.Stroke_CompPheno;

select top 100 * from S19.dbo.Stroke_TEST
order by PATIENT_NUM;
select count(PATIENT_NUM) from S19.dbo.Stroke_TEST;
select count(distinct PATIENT_NUM) from S19.dbo.Stroke_TEST;

select top 100 * from S19.dbo.Stroke_CompPheno
order by PATIENT_NUM;
select count(PATIENT_NUM) from S19.dbo.Stroke_CompPheno;
select count(distinct PATIENT_NUM) from S19.dbo.Stroke_CompPheno;
...
with temp_aggregated as ( 	
	...
	select
		PATIENT_NUM,
		min(START_DATE) as earliest_T2D_date,
		max(START_DATE) as latest_T2D_date,
		count(distinct START_DATE) as N_T2D_codes_diffdates
	from FellowsSample.S19.OBSERVATION_FACT
...
select
  ...
  --count(distinct t.PATIENT_NUM)
  	clean_ObsFact.PATIENT_NUM as PATIENT_NUM,
	clean_ObsFact.ENCOUNTER_NUM as earliest_T2D_encounter_num,
	t.earliest_T2D_date as earliest_T2D_date,
	t.latest_T2D_date as latest_T2D_date,
	t.N_T2D_codes_diffdates,
	clean_ObsFact.CONCEPT_CD as earliest_T2D_code,
	clean_ObsFact.TVAL_CHAR as earliest_T2D_TVAL_CHAR,
	1 as T2D_ComputablePhenotype
into S19.dbo.T2D_CompPheno
from clean_ObsFact
INNER JOIN temp_aggregated t on t.PATIENT_NUM = clean_ObsFact.PATIENT_NUM AND t.earliest_T2D_date = clean_ObsFact.START_DATE
WHERE rn = 1
ORDER BY PATIENT_NUM, earliest_T2D_date;

--drop table S19.dbo.T2D_TEST;
--drop table S19.dbo.T2D_CompPheno;

select top 100 * from S19.dbo.T2D_TEST
order by PATIENT_NUM;
select count(PATIENT_NUM) from S19.dbo.T2D_TEST;
select count(distinct PATIENT_NUM) from S19.dbo.T2D_TEST;

select top 100 * from S19.dbo.T2D_CompPheno
order by PATIENT_NUM;
select count(PATIENT_NUM) from S19.dbo.T2D_CompPheno;
select count(distinct PATIENT_NUM) from S19.dbo.T2D_CompPheno;